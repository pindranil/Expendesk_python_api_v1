...
class InvoicePrompts:
    ...
    def get_shared_extraction_rules():
        return (
            ...
            "3. Use Tax Summary sections for tax fields\n"
        )
    ...
    def get_hotel_prompt():
        return (
            "INVOICE TYPE: HOTEL\n"
            "Extract full structured hotel invoice data.\n\n"
            f"{InvoicePrompts.get_language_instruction()}"
    ...
    def get_travel_prompt():
        return (
            "INVOICE TYPE: TRAVEL\n"
            "Extract travel details (routes, tickets, tax).\n\n"
            f"{InvoicePrompts.get_language_instruction()}"
            f"{InvoicePrompts.get_currency_rules()}"
            f"{InvoicePrompts.get_formatting_rules()}"
            f"{InvoicePrompts.get_shared_extraction_rules()}\n"
            "TRAVEL FIELDS:\n"
            "- travel_type (flight/train/cab), travel_from, travel_to, travel_date\n"
            "- ticket_number, passenger_name, seat_number, class_type\n"
            "- sgst_amount, cgst_amount, igst_amount (if shown)"
        )
    ...
    def get_food_prompt():
        return (
            "INVOICE TYPE: FOOD\n"
            "Extract restaurant/food invoice data.\n\n"
            f"{InvoicePrompts.get_language_instruction()}"
            f"{InvoicePrompts.get_currency_rules()}"
            f"{InvoicePrompts.get_formatting_rules()}"
            f"{InvoicePrompts.get_shared_extraction_rules()}\n"
            ...
            "- alcohol/beverage clearly marked\n"
            "- table_number, order_type (dine-in/takeaway/delivery)\n"
            "- sgst_amount, cgst_amount, service_charge\n"
            f"{InvoicePrompts.get_translation_examples()}"
        )
    ...
    def get_other_prompt():
        return (
            "INVOICE TYPE: OTHER\n"
            "Extract general invoice fields precisely.\n\n"
            f"{InvoicePrompts.get_language_instruction()}"
            f"{InvoicePrompts.get_currency_rules()}"
            f"{InvoicePrompts.get_formatting_rules()}"
            f"{InvoicePrompts.get_shared_extraction_rules()}\n"
            "FIELDS:\n"
            "- merchant_name, invoice_no, invoice_date, total_amount\n"
            "- service_description, item list, taxes (sgst/cgst/igst)"
        )
