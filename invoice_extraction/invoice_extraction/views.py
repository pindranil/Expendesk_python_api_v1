...
class AnalyzeImageView(APIView):
    ...
    async def process_images_async(self, image_files, combined_prompt):
        """Async processing of all validated image files — single API call per image"""
        tasks = [
            self.image_processor.process_single_image(file, combined_prompt)  # ✅ no get_prompt_by_type needed
    ...
    def post(self, request, *args, **kwargs):
        ...
        # ✅ single combined prompt — classify + extract in one shot
        combined_prompt = InvoicePrompts.get_combined_prompt()

        if validated_files:
            print(f"[INFO] Processing {len(validated_files)} files asynchronously")
            async_results = asyncio.run(
                self.process_images_async(validated_files, combined_prompt)
            )
